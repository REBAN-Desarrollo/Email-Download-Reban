...
class GmailDownloaderApp:
    ...
    def log(self, message):
        """Añade un mensaje al log en la interfaz gráfica"""
    ...
    def connect_imap(self):
        ...
        self.mail_conn = imaplib.IMAP4_SSL(self.imap_server)
    ...
    def search_emails(self):
        try:
            ...
            if body_kw: criteria.append(f'TEXT "{body_kw}"') # TEXT busca en asunto y cuerpo
            ...
            for eid in ids:
                ...
                self.tree.insert("", "end", values=(eid.decode(), fecha_str, remitente, asunto))
    ...
    def download_emails(self, items_data):
        try:
            ...
            for idx, item in enumerate(items_data, 1):
                ...
                final_html = body_html or f"<pre>{body_text}</pre>"
                ...
                self.convert_html_to_pdf(pdf_html, pdf_path)
                self.log("  📄 Cuerpo guardado en Mensaje_Legible.pdf")

            self.log("✅ ¡Descarga masiva completada exitosamente!")
            messagebox.showinfo("Éxito", f"Se han descargado {len(items_data)} correos exitosamente en:\n{self.output_dir}")
